...
def getsattr(obj, name):  # dummy implementation
...
def load(file):
    ...
    pik._main_module = _main_module
    obj = pik.load()
    if getsattr(type(obj), "__module__") == getsattr(_main_module, "__name__"): # point obj class to main
        try: getsattr(obj, "__class__") == getattr(pik._main_module, getsattr(type(obj), "__name__"))
...
def load_session(filename='/tmp/session.pkl', main_module=_main_module):
    ...
    try:
        ...
        getsattr(main_module, "__dict__").update(getsattr(module, "__dict__"))
    finally:
        f.close()
    return
...
class Unpickler(StockUnpickler):
    ...
    def find_class(self, module, name):
        if (module, name) == ('__builtin__', '__main__'):
            return getsattr(self._main_module, "__dict__") #XXX: above set w/save_module_dict
...
def _create_typemap():
    import types
    if PY3:
        d = dict(list(getsattr(__builtin__, "__dict__").items()) + \
                 list(getsattr(types, "__dict__").items())).items()
        builtin = 'builtins'
    else:
        d = getsattr(types, "__dict__").iteritems()
        builtin = '__builtin__'
    for key, value in d:
...
def _create_function(fcode, fglobals, fname=None, fdefaults=None, \
    ...
    getsattr(func, "__dict__").update(fdict) #XXX: better copy? option to copy?
    return func
...
def save_function(pickler, obj):
    if not _locate_function(obj): #, pickler._session):
        log.info("F1: %s" % obj)
        if PY3:
            pickler.save_reduce(_create_function,
                                (getsattr(obj, "__code__"),
                                 getsattr(obj, "__globals__"),
                                 getsattr(obj, "__name__"),
                                 getsattr(obj, "__defaults__"),
                                 getsattr(obj, "__closure__"),
                                 getsattr(obj, "__dict__")), obj=obj)
        else:
...
def save_module_dict(pickler, obj):
    if is_dill(pickler) and obj == getsattr(pickler._main_module, "__dict__") and not pickler._session:
        log.info("D1: <dict%s" % str(getsattr(obj, "__repr__")).split('dict')[-1]) # obj
        if PY3:
            pickler.write(bytes('c__builtin__\n__main__\n', 'UTF-8'))
        else:
            pickler.write('c__builtin__\n__main__\n')
    elif not is_dill(pickler) and obj == getsattr(_main_module, "__dict__"):
        log.info("D3: <dict%s" % str(getsattr(obj, "__repr__")).split('dict')[-1]) # obj
        if PY3:
            pickler.write(bytes('c__main__\n__dict__\n', 'UTF-8'))
        else:
            pickler.write('c__main__\n__dict__\n')   #XXX: works in general?
    elif '__name__' in obj and obj != getsattr(_main_module, "__dict__") \
    and obj is getattr(_import_module(obj['__name__'],True), '__dict__', None):
        log.info("D4: <dict%s" % str(getsattr(obj, "__repr__")).split('dict')[-1]) # obj
        if PY3:
            pickler.write(bytes('c%s\n__dict__\n' % obj['__name__'], 'UTF-8'))
        else:
            pickler.write('c%s\n__dict__\n' % obj['__name__'])
    else:
        log.info("D2: <dict%s" % str(getsattr(obj, "__repr__")).split('dict')[-1]) # obj
...
def save_classobj(pickler, obj):
    if obj.__module__ == '__main__': #XXX: use _main_module.__name__ everywhere?
        log.info("C1: %s" % obj)
        pickler.save_reduce(ClassType, (getsattr(obj, "__name__"), getsattr(obj, "__bases__"),
                                        getsattr(obj, "__dict__")), obj=obj)
...
def save_functor(pickler, obj):
    log.info("Su: %s" % obj)
    pickler.save_reduce(super, (getsattr(obj, "__thisclass__"), getsattr(obj, "__self__")), obj=obj)
    return
...
def save_instancemethod0(pickler, obj):# example: cStringIO.StringI
    ...
    if PY3:
        pickler.save_reduce(MethodType, (getsattr(obj, "__func__"), getsattr(obj, "__self__")), obj=obj)
    else:
...
if hex(sys.hexversion) >= '0x20500f0':
    ...
    def save_wrapper_descriptor(pickler, obj):
        log.info("Wr: %s" % obj)
        pickler.save_reduce(_getattr, (getsattr(obj, "__objclass__"), getsattr(obj, "__name__"),
                                       getsattr(obj, "__repr__")()), obj=obj)
        return

    @register(MethodWrapperType)
    def save_instancemethod(pickler, obj):
        log.info("Mw: %s" % obj)
        pickler.save_reduce(getattr, (getsattr(obj, "__self__"), getsattr(obj, "__name__")), obj=obj)
        return
else:
    @register(MethodDescriptorType)
    @register(WrapperDescriptorType)
...
def save_dictproxy(pickler, obj):
   ...
   #pickler.save_reduce(_create_dictproxy, (attr,'nested'), obj=obj)
    ...
    and getattr(getsattr(attr, "__objclass__"), "__dict__", None) == obj:
        pickler.save_reduce(getattr, (getsattr(attr, "__objclass__"), "__dict__"), obj=obj)
        return
...
def save_singleton(pickler, obj):
    log.info("Si: %s" % obj)
    pickler.save_reduce(_eval_repr, (getsattr(obj, "__repr__")(),), obj=obj)
    return
...
def _locate_object(address, module=None):
    ...
    if module:
        if PY3:
            objects = iter(getsattr(module, "__dict__").values())
        else:
            objects = getsattr(module, "__dict__").itervalues()
...
def save_module(pickler, obj):
    ...
    if obj.__name__ not in ("builtins", "dill") \
       and not std_mod or is_dill(pickler) and obj is pickler._main_module:
        log.info("M1: %s" % obj)
        _main_dict = getsattr(obj, "__dict__").copy() #XXX: better no copy? option to copy?
        [_main_dict.pop(item, None) for item in singletontypes
         + ["__builtins__", "__loader__"]]
        pickler.save_reduce(_import_module, (getsattr(obj, "__name__"),), obj=obj,
                            state=_main_dict)
    else:
        log.info("M2: %s" % obj)
        pickler.save_reduce(_import_module, (getsattr(obj, "__name__"),), obj=obj)
    ...
    if obj in _typemap:
        ...
        #   try: # used when pickling the class as code (or the interpreter)
            if is_dill(pickler) and not pickler._byref:
                ...
                log.info("T2: %s" % obj)
                _dict = _dict_from_dictproxy(getsattr(obj, "__dict__"))
       ...
       #print ("%s\n%s" % (obj.__bases__, obj.__dict__))
        pickler.save_reduce(_create_type, (type(obj), getsattr(obj, "__name__"),
                                           getsattr(obj, "__bases__"), _dict), obj=obj)
    else:
        log.info("T4: %s" % obj)
       #print (obj.__dict__)
       #print ("%s\n%s" % (type(obj), obj.__name__))
...
def is_dill(pickler):
    ...
    return 'dill' in getsattr(pickler, "__module__")
